...
print("\n=== [글로벌 기관 투자 변화] ===")
try:
    inst_changes = wa.load_institutional_changes()
except Exception as _e:
    print(f"  [경고] 기관 변화 로드 실패: {_e}")
    inst_changes = []
...
with open(out_path, 'a', encoding='utf-8') as f:
    ...
    for _i, _item in enumerate(inst_changes):
        f.write(f'INST_CHNG_{_i}_FUND={_item["name"]}\n')
        f.write(f'INST_CHNG_{_i}_PERIOD={_item["period"]}\n')
        for _j, (_sec, _delta) in enumerate(_item["top_changes"]):
            _sign = '+' if _delta > 0 else ''
            f.write(f'INST_CHNG_{_i}_S{_j}={_sec}\n')
