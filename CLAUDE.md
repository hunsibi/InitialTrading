...
- `send_telegram.py`는 다른 파이프라인 산출물(weekly_full_*.html)과 무관하게 실행 시점에 직접 라이브 데이터를 조회해
  **요약 메시지 1건 + HTML 리포트 파일 1건**을 전송한다. 데이터는 `collect()`가 한 번만 모아 양쪽이 공유:
  ...
  3. 🇰🇷 삼성전자·SK하이닉스 매매 동향 — **최근 5거래일** 외국인/기관/개인 순매수(억원).
     텔레그램 본문에는 당일 + 5일 누적, 그래프는 HTML 리포트에
     - 1순위: pykrx `get_market_trading_value_by_date` (KRX 공식 **거래대금**, KRX_ID/KRX_PW 필요)
     - 2순위: 네이버 `m.stock.naver.com/api/stock/{code}/trend` (순매수 **수량** × 종가 환산 → 추정치, 로그인 불필요)
     - **KRX는 주기적으로 비밀번호 변경을 강제해 로그인이 끊긴다.** 그때 수급이 통째로 비는 걸 막는 게 폴백의 목적.
       폴백이 쓰이면 텔레그램·HTML 양쪽에 "추정치"라고 명시된다 — 출처를 숨기지 말 것
     - 복구 방법: krx.co.kr에서 비밀번호 변경 → `krx_config.py`와 GitHub Secret `KRX_PW` 동시 갱신
...
- **GitHub Actions — 워크플로 2개로 분리**
  | 파일 | 역할 | cron (KST) |
  |---|---|---|
  | `daily-briefing.yml` | 텔레그램 브리핑 전담 (경량, MongoDB 불필요) | 17:30 / 18:30 / 20:05 / 21:35 — 4회 시도 |
  | `weekly-analysis.yml` | 퀀트 파이프라인 · HTML 리포트 · Issue | 22:20 |

  - **cron이 17:30처럼 이른 이유**: 실측상 1차 cron이 매번 **3시간 30분** 밀렸다(20:37 예약 → 익일 00:07 실행).
    지연을 역산해 이른 시각에 걸어두고, 실제 실행 시각이 **20:00~익일 09:00 KST**일 때만 보내는 구간 가드를 둔다.
    정시에 돌면 17:30·18:30 회차는 건너뛰고 20:05가 보내고, 3.5시간 밀리면 17:30 회차가 약 21:00에 보낸다
  - **20:00 하한**: 넥스트레이드 애프터마켓 종료 시각. 그 전에 보내면 시간외 최종가가 아닌 장중 값이 찍힌다 — 낮추지 말 것
  - **장 기준일(`market_date()`)**: 자정 넘겨 실행되면 달력 날짜는 다음 날이지만 데이터는 전 거래일 것이다.
    09:00 이전이면 전날로, 주말이면 직전 금요일로 당긴다. 마커 키도 이 날짜라 자정 실행이 이중 발송되지 않는다

  - **20:00 이후에 실행하는 이유**: 시간외(넥스트레이드 애프터마켓)가 **20:00에 종료**되므로
    그 뒤여야 시간외 *최종* 체결가가 잡힌다. 이전 17:00 스케줄은 시간외 장중이라 중간값이 찍혔다 — 앞당기지 말 것
  - **재시도가 필요한 이유**: GitHub 예약 실행은 러너 부하 시 지연되거나 **아예 취소(drop)된다**.
    실측 결과 평상시 30~40분 지연이 상시였고, 10시간 넘게 밀리거나(08-27·08-28) 누락된 날(09-01)도 있었다.
    cron 분을 :00/:30에서 비켜둔 것도 congestion 회피 목적 — 정각으로 되돌리지 말 것
  - **중복 방지**: `actions/cache`에 `briefing-sent-<KST날짜>` 마커를 남겨, 같은 날 두 번째 회차는 건너뛴다.
    전송 실패 시엔 마커를 남기지 않아 다음 회차가 재시도한다. `workflow_dispatch`(수동)는 마커를 무시하고 강제 전송
  - **이중 발송 금지**: `weekly-analysis.yml`은 반드시 `SKIP_TELEGRAM: '1'` 유지 — 빼면 하루 두 번 간다
